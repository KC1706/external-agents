...
Relative tool paths require an explicit absolute `cwd` or `workdir` in the tool arguments. Without that anchor, the observer skips attribution because a gateway task may use a different working directory from the Hermes process. Absolute file targets remain supported. Nested repository exclusions never fall back to a parent registration.
